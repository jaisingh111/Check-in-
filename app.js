...
function renderCalendar(){
  ...
  for (let i=0;i<42;i++){
    ...
    `;
    const onDayTap = (ev) => {
      // Keep this synchronous to preserve the user-gesture for iOS Safari.
      // Also avoid a double-trigger on iOS where touchend is followed by click.
      if (ev.type === 'click' && window.__lastDayTouchTs && Date.now() - window.__lastDayTouchTs < 450) return;
      if (ev.type === 'touchend') window.__lastDayTouchTs = Date.now();
      ev.preventDefault();
      ev.stopPropagation();
      openDayActions(dayId);
    };
    cell.addEventListener("click", onDayTap);
    // Some iOS Safari/PWA cases behave more reliably with touchend.
    cell.addEventListener(
      "touchend",
      (ev) => {
        // Avoid double-firing when click also triggers.
...
// --- Day tap menu (View / Take photo) ---
let dayActionEl = null;
let dayViewerEl = null;

function ensureDayOverlays(){
  if (dayActionEl) return;

  const style = document.createElement('style');
  style.textContent = `
    .dayActionOverlay{position:fixed;inset:0;z-index:9999;background:rgba(0,0,0,.45);display:none;align-items:flex-end;justify-content:center;padding:18px}
    .dayActionSheet{width:min(520px,100%);background:rgba(25,18,45,.92);border:1px solid rgba(255,255,255,.12);border-radius:18px;padding:14px;backdrop-filter:blur(10px)}
    .dayActionTitle{font-weight:700;margin:2px 0 10px 2px;color:rgba(255,255,255,.92)}
    .dayActionBtns{display:flex;flex-direction:column;gap:10px}
    .dayActionBtns button{width:100%;padding:12px 14px;border-radius:12px;border:1px solid rgba(255,255,255,.12);background:rgba(255,255,255,.07);color:rgba(255,255,255,.92);font-weight:700}
    .dayActionBtns button:disabled{opacity:.45}
    .dayActionBtns button.primary{background:rgba(120,170,255,.22)}
    .dayViewerOverlay{position:fixed;inset:0;z-index:10000;background:rgba(0,0,0,.6);display:none;align-items:center;justify-content:center;padding:18px}
    .dayViewer{width:min(780px,100%);max-height:90vh;overflow:auto;background:rgba(25,18,45,.92);border:1px solid rgba(255,255,255,.12);border-radius:18px;padding:14px;backdrop-filter:blur(10px)}
    .dayViewerTop{display:flex;align-items:center;justify-content:space-between;gap:12px;margin-bottom:10px}
    .dayViewerTop h3{margin:0;font-size:16px;color:rgba(255,255,255,.92)}
    .dayViewerTop button{padding:8px 10px;border-radius:10px;border:1px solid rgba(255,255,255,.12);background:rgba(255,255,255,.07);color:rgba(255,255,255,.92);font-weight:700}
    .dayViewerGrid{display:grid;grid-template-columns:1fr;gap:12px}
    .dayViewerCard{border:1px solid rgba(255,255,255,.12);border-radius:14px;padding:10px;background:rgba(255,255,255,.05)}
    .dayViewerCard .label{font-weight:700;margin-bottom:8px;color:rgba(255,255,255,.85)}
    .dayViewerCard img{width:100%;height:auto;border-radius:12px;display:block}
    @media (min-width:700px){.dayViewerGrid{grid-template-columns:1fr 1fr}}
  `;
  document.head.appendChild(style);

  dayActionEl = document.createElement('div');
  dayActionEl.className = 'dayActionOverlay';
  dayActionEl.innerHTML = `
    <div class="dayActionSheet" role="dialog" aria-modal="true">
      <div class="dayActionTitle">Day options</div>
      <div class="dayActionBtns">
        <button type="button" class="primary" data-act="take">Take / Replace photo</button>
        <button type="button" data-act="view">View photo</button>
        <button type="button" data-act="cancel">Cancel</button>
      </div>
    </div>
  `;
  document.body.appendChild(dayActionEl);
  dayActionEl.addEventListener('click', (e) => {
    if (e.target === dayActionEl) hideDayActions();
  });

  dayViewerEl = document.createElement('div');
  dayViewerEl.className = 'dayViewerOverlay';
  dayViewerEl.innerHTML = `
    <div class="dayViewer" role="dialog" aria-modal="true">
      <div class="dayViewerTop">
        <h3>Photos</h3>
        <button type="button" data-close="1">Close</button>
      </div>
      <div class="dayViewerGrid" data-grid="1"></div>
    </div>
  `;
  document.body.appendChild(dayViewerEl);
  dayViewerEl.querySelector('[data-close="1"]').addEventListener('click', hideDayViewer);
  dayViewerEl.addEventListener('click', (e) => {
    if (e.target === dayViewerEl) hideDayViewer();
  });
}

function getDayDocFromCache(dayId){
  const key = getMonthKey(viewYear, viewMonth);
  const m = monthCache.get(key);
  return m ? (m[dayId] || null) : null;
}

let activeDayActionsId = null;

function openDayActions(dayId){
  if (!storage || !db) {
    alert('Open Setup and save your Firebase config + Room ID first.');
    return;
  }
  ensureDayOverlays();
  activeDayActionsId = dayId;

  const doc = getDayDocFromCache(dayId) || {};
  const hasAny = !!(doc.husbandUrl || doc.wifeUrl);

  const viewBtn = dayActionEl.querySelector('button[data-act="view"]');
  viewBtn.disabled = !hasAny;

  // Wire button actions (re-wire each open to keep it simple)
  dayActionEl.querySelector('button[data-act="take"]').onclick = () => {
    hideDayActions();
    // This click is the user gesture we need for iOS Safari.
    openCameraForDay(dayId);
  };
  viewBtn.onclick = () => {
    hideDayActions();
    openDayViewer(dayId);
  };
  dayActionEl.querySelector('button[data-act="cancel"]').onclick = hideDayActions;

  dayActionEl.style.display = 'flex';
}

function hideDayActions(){
  if (dayActionEl) dayActionEl.style.display = 'none';
  activeDayActionsId = null;
}

function openDayViewer(dayId){
  ensureDayOverlays();
  const doc = getDayDocFromCache(dayId) || {};
  const grid = dayViewerEl.querySelector('[data-grid="1"]');
  grid.innerHTML = '';

  const items = [];
  if (doc.husbandUrl) items.push({ label: 'Husband', url: doc.husbandUrl });
  if (doc.wifeUrl) items.push({ label: 'Wife', url: doc.wifeUrl });

  if (items.length === 0) {
    const empty = document.createElement('div');
    empty.style.color = 'rgba(255,255,255,.8)';
    empty.textContent = 'No photo saved for this day yet.';
    grid.appendChild(empty);
  } else {
    for (const it of items) {
      const card = document.createElement('div');
      card.className = 'dayViewerCard';
      card.innerHTML = `<div class="label">${it.label}</div>`;
      const img = document.createElement('img');
      img.src = it.url;
      img.alt = `${it.label} photo for ${dayId}`;
      img.loading = 'lazy';
  ...
  dayViewerEl.style.display = 'flex';
}

function hideDayViewer(){
  if (dayViewerEl) dayViewerEl.style.display = 'none';
}
function openCameraForDay(dayId){
  ...
  // IMPORTANT: do not await anything before calling input.click().
  // Safari will block the camera/file picker if the user-gesture chain is broken.
  pendingDayId = dayId;
  cameraInput.value = "";
  cameraInput.accept = "image/*";
  // Helps iOS show the camera option (still allows choosing from library)
