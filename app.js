...
function openViewer(dayId, data) {
  ...
  vTitle.textContent = `Photos & notes for ${dayId}`;

  // Always read latest cached data (snapshot listener keeps this updated)
  const latest = monthCache.get(dayId) || data || {};

  const parts = [];
  if (latest.husbandUrl) {
    parts.push(`<div class="dc-label">Husband</div><img class="dc-img" src="${latest.husbandUrl}" alt="Husband photo for ${dayId}">`);
  }
  if (latest.wifeUrl) {
    parts.push(`<div class="dc-label">Wife</div><img class="dc-img" src="${latest.wifeUrl}" alt="Wife photo for ${dayId}">`);
  }
  if (!parts.length) {
    parts.push(`<div style="opacity:.85;">No photo saved for this day yet.</div>`);
  }

  // Notes (private to your room – visible to both of you)
  const myField = activeRole === "husband" ? "husbandNote" : "wifeNote";
  const theirField = activeRole === "husband" ? "wifeNote" : "husbandNote";
  const myLabel = activeRole === "husband" ? "Your note (Husband)" : "Your note (Wife)";
  const theirLabel = activeRole === "husband" ? "Wife’s note" : "Husband’s note";

  const myNote = (latest[myField] || "");
  const theirNote = (latest[theirField] || "");

  parts.push(`
    <div class="dc-notes">
      <div class="dc-notes-title">Notes</div>

      <div class="dc-note-block">
        <div class="dc-note-label">${escapeHtml(myLabel)}</div>
        <textarea id="dcMyNote" class="dc-note-input" rows="3" placeholder="Write something for this day…">${escapeHtml(myNote)}</textarea>
        <div class="dc-note-actions">
          <div id="dcNoteStatus" class="dc-note-status"></div>
          <button id="dcNoteClear" type="button" class="dc-btn">Clear</button>
          <button id="dcNoteSave" type="button" class="dc-btn primary">Save</button>
        </div>
      </div>

      <div class="dc-note-block">
        <div class="dc-note-label">${escapeHtml(theirLabel)}</div>
        <div class="dc-note-readonly" id="dcTheirNote">${theirNote ? escapeHtml(theirNote).replace(/
/g,"<br>") : `<span class="muted">No note yet.</span>`}</div>
      </div>
    </div>
  `);
  ...
  // Wire note saving (safe + isolated)
  const ta = document.getElementById("dcMyNote");
  const status = document.getElementById("dcNoteStatus");
  const btnSave = document.getElementById("dcNoteSave");
  const btnClear = document.getElementById("dcNoteClear");

  let debounceTimer = null;
  async function saveNow() {
    if (!db || !roomId) {
      status.textContent = "Not connected (setup?)";
      return;
    }
    ...
    try {
      await setDoc(
        ...
        { [myField]: text, updatedAt: serverTimestamp() },
        { merge: true }
      );
      ...
      console.error(err);
      status.textContent = "Couldn’t save (rules?)";
    }
  }
  ...
  function scheduleSave() {
    ...
    debounceTimer = setTimeout(saveNow, 650);
  }

  ta.addEventListener("input", scheduleSave);
  btnSave.addEventListener("click", () => { clearTimeout(debounceTimer); saveNow(); });
  btnClear.addEventListener("click", () => { ta.value = ""; clearTimeout(debounceTimer); saveNow(); });
}
