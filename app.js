...
function renderCalendar(){
  ...
  for (let i=0;i<42;i++){
    ...
    cell.addEventListener("click", async (e) => {
      // Require role selection so we know which photo you mean.
      if (!selectedRole) {
        alert("Select Husband or Wife first.");
        return;
      }

      // Check if a photo already exists for this day + selected role.
      const cached = monthCache.get(dayId) || null;
      const existingUrl = getPhotoUrlFromDayDoc(cached, selectedRole);

      if (existingUrl) {
        const wantView = window.confirm(
          "A photo already exists for this day.\n\nOK = View photo\nCancel = Take/replace photo"
        );
        if (wantView) {
          openPhotoViewer(existingUrl);
          return;
        }
      }
...
// --- Day tap helpers (view vs take) ---
function getPhotoUrlFromDayDoc(dayDoc, role) {
  if (!dayDoc) return null;
  const r = String(role || "").toLowerCase();
  if (r === "husband") return dayDoc.husbandUrl || null;
  if (r === "wife") return dayDoc.wifeUrl || null;
  return null;
}
...
function openPhotoViewer(url) {
  ...
  let overlay = document.getElementById("photoViewerOverlay");
  if (!overlay) {
    overlay = document.createElement("div");
    overlay.id = "photoViewerOverlay";
    overlay.className = "photo-viewer";
    overlay.innerHTML = `
      <div class="photo-viewer__backdrop" data-close="1"></div>
      <div class="photo-viewer__panel" role="dialog" aria-modal="true">
        <button class="photo-viewer__close" type="button" data-close="1">Close</button>
        <img class="photo-viewer__img" alt="Photo" />
      </div>
    `;
  ...
  const img = overlay.querySelector(".photo-viewer__img");
  if (img) img.src = url;
