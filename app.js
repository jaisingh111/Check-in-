...
function ensureNoteViewer() {
  ...
  overlay.innerHTML = `
    <div class="dc-view" role="dialog" aria-modal="true">
      <div class="dc-view-header">
        <div class="dc-view-title" id="noteTitle">Daily Note</div>
        <button class="dc-close" id="noteCloseBtn" aria-label="Close">✕</button>
      </div>
        ...
        <div class="dc-notes">
          <div class="dc-notes-grid">
            <div class="dc-note-card">
              <div class="dc-note-title">Husband note (private)</div>
              <textarea id="noteHText" class="dc-note-input" rows="4" placeholder="Write something for this day..."></textarea>
              <div class="dc-note-actions">
                <button id="noteHSave" type="button" class="btn">Save note</button>
                <span id="noteHSaved" class="dc-note-saved"></span>
              </div>
              <div class="mood-hint" id="noteHHint"></div>
            </div>

            <div class="dc-note-card">
              <div class="dc-note-title">Wife note (private)</div>
              <textarea id="noteWText" class="dc-note-input" rows="4" placeholder="Write something for this day..."></textarea>
              <div class="dc-note-actions">
                <button id="noteWSave" type="button" class="btn">Save note</button>
                <span id="noteWSaved" class="dc-note-saved"></span>
              </div>
...
function openNoteViewer(dayId, data = {}) {
  ...
  title.textContent = `Daily Note — ${dayId}`;
  ...
  // Populate
  if (hText) hText.value = (data.husbandNote || "");
  if (wText) wText.value = (data.wifeNote || "");

  const husbandCanEdit = activeRole === "husband";
  const wifeCanEdit = activeRole === "wife";

  // Permissions
  if (hText) hText.disabled = !husbandCanEdit;
  if (hSave) hSave.disabled = !husbandCanEdit;
  if (hHint) hHint.textContent = husbandCanEdit ? "" : "Only Husband can edit this.";

  if (wText) wText.disabled = !wifeCanEdit;
  if (wSave) wSave.disabled = !wifeCanEdit;
  if (wHint) wHint.textContent = wifeCanEdit ? "" : "Only Wife can edit this.";
