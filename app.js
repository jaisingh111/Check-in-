...
function qaFieldForRoleChar(roleChar){
  return roleChar === 'h' ? 'husbandAnswers' : 'wifeAnswers';
}
function qaRoleName(roleChar){
  return roleChar === 'h' ? 'husband' : 'wife';
}
function qaTextarea(roleChar, qNum){
  return document.getElementById(`${roleChar}_q${qNum}`);
}
function qaHint(roleChar, qNum){
  return document.querySelector(`.saveHint[data-role="${roleChar}"][data-q="${qNum}"]`);
}

async function saveSingleQA(roleChar, qNum){
  if(!db || !ROOM_ID) return;
  if(!selectedDayId) selectedDayId = dayIdFromDate(new Date());
  const ta = qaTextarea(roleChar, qNum);
  if(!ta) return;
  const field = qaFieldForRoleChar(roleChar);
  const key = 'q' + qNum;
  const val = ta.value || '';
  const ref = dayRefFor(selectedDayId);
  const hint = qaHint(roleChar, qNum);
  try{
    await setDoc(ref, { dayId: selectedDayId, [field]: { [key]: val } }, { merge: true });
    if(hint){
      hint.textContent = 'Saved ✓';
      setTimeout(() => { if(hint.textContent === 'Saved ✓') hint.textContent = ''; }, 1200);
    }
  }catch(e){
    console.error(e);
...
async function clearSingleQA(roleChar, qNum){
  const ta = qaTextarea(roleChar, qNum);
  if(ta) ta.value = '';
  await saveSingleQA(roleChar, qNum);
}
...
function wireQA(){
  // autosave on typing (still works)
  hQ.forEach(t => t && t.addEventListener('input', () => scheduleSave('husband')));
  wQ.forEach(t => t && t.addEventListener('input', () => scheduleSave('wife')));

  // Per-question Save / Clear
  document.querySelectorAll('button.saveQA').forEach(btn => {
    btn.addEventListener('click', async () => {
      const roleChar = btn.getAttribute('data-role'); // 'h' or 'w'
      const qNum = btn.getAttribute('data-q');        // '1'..'4'
      await saveSingleQA(roleChar, qNum);
    });
  });

  document.querySelectorAll('button.clearQA').forEach(btn => {
    btn.addEventListener('click', async () => {
      const roleChar = btn.getAttribute('data-role'); // 'h' or 'w'
      const qNum = btn.getAttribute('data-q');        // '1'..'4'
      await clearSingleQA(roleChar, qNum);
    });
  });

  // Clear ALL (bottom button) keeps working
