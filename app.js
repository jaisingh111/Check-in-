...
function openViewer(dayId, data) {
  ...
  if (data.husbandUrl) {
    parts.push(
      `<div class="dc-label">Husband</div><img class="dc-img" src="${data.husbandUrl}" alt="Husband photo for ${dayId}">`
    );
  }
  if (data.wifeUrl) {
    parts.push(
      `<div class="dc-label">Wife</div><img class="dc-img" src="${data.wifeUrl}" alt="Wife photo for ${dayId}">`
    );
  }
  if (!data.husbandUrl && !data.wifeUrl) {
    parts.push(`<div style="opacity:.85;">No photo saved for this day yet.</div>`);
  }

  // Notes (2A: two notes, both visible)
  const myRoleLabel = activeRole === "husband" ? "Husband" : "Wife";
  const otherRoleLabel = activeRole === "husband" ? "Wife" : "Husband";
  const myNoteKey = activeRole === "husband" ? "husbandNote" : "wifeNote";
  const otherNoteKey = activeRole === "husband" ? "wifeNote" : "husbandNote";
  const myNoteVal = (data && data[myNoteKey]) ? String(data[myNoteKey]) : "";
  const otherNoteVal = (data && data[otherNoteKey]) ? String(data[otherNoteKey]) : "";
  ...
  parts.push(`
    <div class="dc-notes">
      ...
      <div class="dc-note-card">
        <div class="dc-note-head">
          <div class="dc-note-label">${myRoleLabel} note</div>
          <button class="dc-note-save" id="dcNoteSaveBtn" type="button">Save</button>
        </div>
        <textarea class="dc-note-text" id="dcMyNote" placeholder="Write something for this day…"></textarea>
        <div class="dc-note-status" id="dcNoteStatus"></div>
      </div>

      <div class="dc-note-card">
        <div class="dc-note-head">
          <div class="dc-note-label">${otherRoleLabel} note</div>
        </div>
        <textarea class="dc-note-text dc-note-text--readonly" id="dcOtherNote" readonly placeholder="No note yet…"></textarea>
      </div>
    </div>
  `);
  ...
  // Fill note values safely
  const myTa = document.getElementById("dcMyNote");
  const otherTa = document.getElementById("dcOtherNote");
  if (myTa) myTa.value = myNoteVal;
  if (otherTa) otherTa.value = otherNoteVal;

  // Wire note save
  const saveBtn = document.getElementById("dcNoteSaveBtn");
  const statusEl = document.getElementById("dcNoteStatus");

  if (saveBtn && myTa) {
    saveBtn.addEventListener("click", async () => {
      const val = (myTa.value || "").trim();
      saveBtn.disabled = true;
      statusEl.textContent = "Saving…";
      try{
        await setDoc(dayDoc(dayId), { [myNoteKey]: val, notesUpdatedAt: serverTimestamp() }, { merge: true });

        // Update cache so reopening keeps it instantly
        const cached = monthCache.get(dayId) || {};
        monthCache.set(dayId, { ...cached, [myNoteKey]: val });
...
async function enablePushNotifications(){
  ...
  try{
    ...
    console.error(e);
    toast("Couldn’t enable notifications. See error details.");
    try{alert("Couldn’t enable notifications.\n\nMost common cause: Firestore rules blocking writes to /rooms/{roomId}/pushSubs.\n\nError: " + (e && e.message ? e.message : e));}catch(_){ }
  }
}
