...
function getStorageBucketName(firebaseConfig) {
  ...
  if (typeof sb === 'string' && sb.trim()) {
    ...
    if (trimmed.includes('appspot.com')) return trimmed;
    // If they pasted a domain, ignore it and derive the bucket.
  }
...
cameraInput.addEventListener("change", async () => {
  ...
  try{
    ...
    let extra = "";
    if (message.includes("CORS") || message.includes("blocked by CORS")) {
      extra = "\n\nThis usually means the Firebase config in Setup is wrong (often authDomain/storageBucket were edited). Open Setup and paste the config EXACTLY from Firebase Console → Project settings → Your apps → Web app.";
    }
    if (code.includes("storage/unauthorized") || code.includes("permission") || code.includes("unauth")) {
      extra = "\n\nThis looks like a permissions/auth issue. Make sure Authentication → Sign-in method → Anonymous is ENABLED, and Storage Rules allow writes for authenticated users (request.auth != null) under your Room ID.";
    }

    alert(`Upload failed${code ? ` (${code})` : ""}.\n${message}${extra}`);
  }
});
