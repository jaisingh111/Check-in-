...
// UK (Europe/London) day-id helpers (handles BST/GMT automatically)
const UK_TZ = "Europe/London";
function ukDayIdFromDate(date = new Date()){
  const parts = new Intl.DateTimeFormat("en-GB", { timeZone: UK_TZ, year: "numeric", month: "2-digit", day: "2-digit" }).formatToParts(date);
  const y = parts.find(p=>p.type==="year")?.value;
  const m = parts.find(p=>p.type==="month")?.value;
  const d = parts.find(p=>p.type==="day")?.value;
  return `${y}-${m}-${d}`;
}
function dayIdToUtcMillis(dayId){
  const [y,m,d] = dayId.split("-").map(Number);
  return Date.UTC(y, m-1, d);
}
function prevDayId(dayId){
  const ms = dayIdToUtcMillis(dayId) - 24*60*60*1000;
...
function computeStreakFromDocs(docsById){
  // docsById: Map<dayId, {husbandUrl?, wifeUrl?, ...}>
  const hasBoth = (id) => {
    const d = docsById.get(id);
    return !!(d && d.husbandUrl && d.wifeUrl);
  };

  const ukTodayId = ukDayIdFromDate(new Date());

  // Current streak counts completed consecutive days ending on:
  // - today, if both photos exist for today
  // - otherwise yesterday (streak stays alive until midnight UK time)
  let startId = hasBoth(ukTodayId) ? ukTodayId : prevDayId(ukTodayId);

  let current = 0;
  let cursor = startId;
  while (hasBoth(cursor)) {
    current++;
    cursor = prevDayId(cursor);
  }

  // Longest streak across all days we have in docsById
  const bothIds = [];
  docsById.forEach((v, k) => {
    if (v && v.husbandUrl && v.wifeUrl) bothIds.push(k);
  });
  bothIds.sort(); // YYYY-MM-DD sorts correctly

  let best = 0;
  let run = 0;
  let prev = null;
  for (const id of bothIds) {
    if (prev && id === nextDayId(prev)) run += 1;
    else run = 1;
...
function attachStreakListener(){
  ...
  const daysCol = collection(db, "rooms", ROOM_ID, "days");
  const q = query(daysCol, orderBy(documentId(), "desc"), limit(800));
