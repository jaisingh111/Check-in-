...
import {getFirestore,
  ...
  where,
  orderBy,
  limit, deleteField} from "https://www.gstatic.com/firebasejs/10.13.0/firebase-firestore.js";
...
// Save / clear a single answer (manual buttons)
const QA_KEYS = ['sleep','mood','mind','help'];

function qaTextarea(roleShort, qNum){
  return document.getElementById(`${roleShort}_q${qNum}`);
}
function qaHint(roleShort, qNum){
  return document.getElementById(`${roleShort}_q${qNum}_hint`);
}

async function saveOneQA(roleShort, qNum){
  const t = qaTextarea(roleShort, qNum);
  if(!t) return;
  const dayId = selectedDayId();
  const roomId = getRoomId();
  if(!dayId || !roomId) return alert('Pick a day + check Setup.');
  const key = QA_KEYS[qNum-1] || `q${qNum}`;
  const field = roleShort === 'h' ? 'husbandAnswers' : 'wifeAnswers';
  const hint = qaHint(roleShort, qNum);
  try{
    const payload = { [field]: { [key]: t.value || '' }, qaUpdatedAt: serverTimestamp() };
    await setDoc(dayRefFor(dayId), payload, { merge: true });
    if(hint){ hint.textContent = 'Saved ✓'; setTimeout(()=>{ hint.textContent=''; }, 2000); }
  }catch(err){
    console.error('QA save failed', err);
    alert('Could not save. Check Firestore rules + setup.');
  }
}

async function clearOneQA(roleShort, qNum){
  const t = qaTextarea(roleShort, qNum);
  if(!t) return;
  t.value = '';
  const dayId = selectedDayId();
  ...
  const hint = qaHint(roleShort, qNum);
  try{
    const payload = { [field]: { [key]: deleteField() }, qaUpdatedAt: serverTimestamp() };
    await setDoc(dayRefFor(dayId), payload, { merge: true });
    if(hint){ hint.textContent = 'Cleared'; setTimeout(()=>{ hint.textContent=''; }, 2000); }
  }catch(err){
    console.error('QA clear failed', err);
    alert('Could not clear. Check Firestore rules + setup.');
  }
}
...
function wireQA(){
  // Manual save/clear buttons per question
  ...
  // Keep the big Clear buttons (clear all for that role)
