...
// ---------------- Year in Review ----------------
let __lastReviewDocsById = null;
let __lastReviewYear = null;
...
async function buildYearReview(){
  ...
  __lastReviewDocsById = docsById;
  __lastReviewYear = year;
...
function pickMonthlySharedDays(year, docsById){
  // One "both" day per month (random within month) for a nice keepsake
  const picks = [];
  for (let m=0; m<12; m++){
    const daysInMonth = new Date(year, m+1, 0).getDate();
    const candidates = [];
    for (let d=1; d<=daysInMonth; d++){
      const id = `${year}-${String(m+1).padStart(2,"0")}-${String(d).padStart(2,"0")}`;
      const data = docsById[id];
      if (data && data.husbandUrl && data.wifeUrl) candidates.push(id);
    }
...
function shouldUsePrintExport(){
  // iOS (especially Home Screen apps) is restrictive with image-to-PDF embedding.
  // Printing uses the OS PDF pipeline and works reliably.
  return isIOS();
}

function buildPrintHtml(year, stats, memoryIds, docsById){
  const esc = (s) => String(s).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;");
  const { both, photos, longest, bestMonth } = stats;
  let rows = "";
  for (const id of memoryIds){
    const d = docsById[id] || {};
    const h = d.husbandUrl ? `<img src="${esc(d.husbandUrl)}" alt="Husband ${esc(id)}">` : `<div>Husband image unavailable</div>`;
    const w = d.wifeUrl ? `<img src="${esc(d.wifeUrl)}" alt="Wife ${esc(id)}">` : `<div>Wife image unavailable</div>`;
    rows += `
      <div class="row">
        <div class="date">${esc(id)}</div>
        ${h}
        ${w}
      </div>
    `;
  ...
  return `
    <h1>Year in Review — ${esc(year)}</h1>
    <div class="sub">Daily Check‑In</div>
    <div class="stats">
      <div class="stat"><div class="k">Both checked‑in</div><div class="v">${esc(both)}</div></div>
      <div class="stat"><div class="k">Total photos</div><div class="v">${esc(photos)}</div></div>
      <div class="stat"><div class="k">Longest “both” streak</div><div class="v">${esc(longest)}</div></div>
      <div class="stat"><div class="k">Most active month</div><div class="v">${esc(bestMonth)}</div></div>
    </div>
    <div class="memTitle">Memories (random shared pick per month)</div>
    <div class="memGrid">
      ${rows || `<div class="sub">No shared check‑ins found for this year yet ❤️</div>`}
    </div>
  `;
}

async function exportYearReviewViaPrint(){
  const year = __lastReviewYear || Number(yearSel.value) || new Date().getFullYear();
  const docsById = __lastReviewDocsById;
  if (!docsById){
    alert("Open Year in Review first, then tap Export PDF.");
    return;
  }
  const stats = {
    both: (statBoth && statBoth.textContent) ? statBoth.textContent : "—",
    photos: (statPhotos && statPhotos.textContent) ? statPhotos.textContent : "—",
    longest: (statLongest && statLongest.textContent) ? statLongest.textContent : "—",
    bestMonth: (statBestMonth && statBestMonth.textContent) ? statBestMonth.textContent : "—",
  };
  const memoryIds = pickMonthlySharedDays(year, docsById);
  if (printArea){
    printArea.innerHTML = buildPrintHtml(year, stats, memoryIds, docsById);
...
async function exportYearReviewPdf(){
  if (shouldUsePrintExport()){
    await exportYearReviewViaPrint();
    return;
  }
  const year = __lastReviewYear || Number(yearSel.value) || new Date().getFullYear();
  const docsById = __lastReviewDocsById;
  ...
  exportPdfBtn && (exportPdfBtn.textContent = "Exporting…");
  ...
  try{
    ...
    // Stats (re-use what is on screen)
    const both = (statBoth && statBoth.textContent) ? statBoth.textContent : "—";
    const photos = (statPhotos && statPhotos.textContent) ? statPhotos.textContent : "—";
    const longest = (statLongest && statLongest.textContent) ? statLongest.textContent : "—";
    const bestMonth = (statBestMonth && statBestMonth.textContent) ? statBestMonth.textContent : "—";

    // Cover
    doc.setFont("helvetica", "bold");
    doc.setFontSize(22);
    doc.text(`Year in Review — ${year}`, margin, 70);
    doc.setFont("helvetica", "normal");
    doc.setFontSize(12);
    doc.text(`Both checked-in: ${both}`, margin, 105);
    doc.text(`Total photos: ${photos}`, margin, 125);
    doc.text(`Longest “both” streak: ${longest}`, margin, 145);
    doc.text(`Most active month: ${bestMonth}`, margin, 165);

    doc.setFontSize(11);
    doc.text("Memories (random pick per month where you both checked-in):", margin, 200);

    // Memory pages: 3 rows per page, each row has husband+wife images
    const picks = pickMonthlySharedDays(year, docsById);
    if (!picks.length){
      doc.text("No shared check-ins found for this year yet ❤️", margin, 230);
    } else {
      let y = 230;
      const gap = 16;
      const rowH = 150;
      ...
      for (let i=0; i<picks.length; i++){
        ...
        const hUrl = await fetchAsDataUrl(data.husbandUrl);
        const wUrl = await fetchAsDataUrl(data.wifeUrl);

        const imgY = y + 10;
        if (hUrl){
          doc.addImage(hUrl, "PNG", margin, imgY, imgW, imgH);
        } else {
          doc.setFont("helvetica", "normal");
          doc.setFontSize(10);
          doc.text("Husband image unavailable", margin, imgY + 20);
        }

        if (wUrl){
          doc.addImage(wUrl, "PNG", margin + imgW + gap, imgY, imgW, imgH);
        } else {
          doc.setFont("helvetica", "normal");
          doc.setFontSize(10);
          doc.text("Wife image unavailable", margin + imgW + gap, imgY + 20);
        }
    ...
    const filename = `Year-in-Review-${year}.pdf`;

    // iOS-friendly share flow
    const file = new File([blob], filename, { type: "application/pdf" });

    if (navigator.share && navigator.canShare && navigator.canShare({ files: [file] })) {
      await navigator.share({ files: [file], title: filename });
    } else {
      // Fallback: open in a new tab, user can share/save
      ...
      alert("PDF generated. If it didn’t download automatically on iPhone, use the Share button in Safari to ‘Save to Files’.");
    }
  } catch(e){
    console.error(e);
    alert("Sorry — the PDF export failed. Try again on a strong connection, or export from a laptop/desktop.");
  } finally {
    exportPdfBtn && (exportPdfBtn.disabled = false);
    exportPdfBtn && (exportPdfBtn.textContent = "Export PDF");
  }
}
...
if (exportPdfBtn){
  exportPdfBtn.addEventListener("click", async () => {
    try{
      await authReady;
      // Ensure latest data
      if (!__lastReviewDocsById || (__lastReviewYear !== Number(yearSel.value))){
        await buildYearReview();
      }
      await exportYearReviewPdf();
    } catch(e){
      console.error(e);
      alert("Couldn’t export. Please open Year in Review first and try again.");
    }
  });
}
