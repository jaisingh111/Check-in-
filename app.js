  ...
  overlay.innerHTML = `
    <div class="dc-sheet" role="dialog" aria-modal="true">
      ...
      <div class="row" style="margin-bottom:10px;">
        ...
        <button class="dc-btn" id="dcBtnNote" type="button">Daily Note</button>
      </div>
      <div class="row">
...
function handleDayTap(dayId) {
  ...
  const btnNote = document.getElementById("dcBtnNote");
  ...
  if (btnNote) {
    btnNote.onclick = () => {
      hideDayMenu();
      openNoteViewer(__pendingDayId, data);
    };
  }
...
function ensureMoodViewer() {
  ...
  // Use the same modal system as Photo/Note viewers (.dc-view + .show)
  const overlay = document.createElement("div");
  overlay.id = "moodOverlay";
  overlay.className = "dc-view";
  overlay.innerHTML = `
    <div class="dc-view-card" role="dialog" aria-modal="true">
      <div class="dc-view-top">
        <div style="font-weight:900;" id="moodTitle">Sleep Emoji Mood</div>
        <button class="dc-x" id="moodCloseBtn" aria-label="Close" type="button">✕</button>
      </div>
      ...
      <div id="moodBody">
        <div class="mood-grid">
          <div class="mood-card">
            ...
            <textarea id="moodHText" class="mood-text" rows="2" placeholder="How did you sleep? (optional)"></textarea>
            <button id="moodHSave" class="dc-btn primary" type="button">Save</button>
          ...
          <div class="mood-card">
            ...
            <textarea id="moodWText" class="mood-text" rows="2" placeholder="How did you sleep? (optional)"></textarea>
            <button id="moodWSave" class="dc-btn primary" type="button">Save</button>
  ...
  document.getElementById("moodCloseBtn").addEventListener("click", hideMoodViewer);
  overlay.addEventListener("click", (e) => { if (e.target === overlay) hideMoodViewer(); });
}
...
function ensureNoteViewer() {
  ...
  // Use the same modal system as Photo/Mood viewers (.dc-view + .show)
  const overlay = document.createElement("div");
  ...
  overlay.innerHTML = `
    <div class="dc-view-card" role="dialog" aria-modal="true">
      <div class="dc-view-top">
        <div style="font-weight:900;" id="noteTitle">Daily Note</div>
        <button class="dc-x" id="noteCloseBtn" aria-label="Close" type="button">✕</button>
      </div>

      <div class="dc-view-body">
        <div class="mood-grid">
          <div class="mood-card">
            <div class="mood-card-title">Husband note (private)</div>
            <textarea id="noteHText" class="mood-text" rows="4" placeholder="Write your note..."></textarea>
            <div class="row" style="margin-top:10px;align-items:center;justify-content:space-between;">
              <button id="noteHSave" class="dc-btn primary" type="button">Save note</button>
              <div class="mood-hint" id="noteHSaved"></div>
            </div>
            <div class="mood-hint" id="noteHHint"></div>
          </div>

          <div class="mood-card">
            <div class="mood-card-title">Wife note (private)</div>
            <textarea id="noteWText" class="mood-text" rows="4" placeholder="Write your note..."></textarea>
            <div class="row" style="margin-top:10px;align-items:center;justify-content:space-between;">
              <button id="noteWSave" class="dc-btn primary" type="button">Save note</button>
              <div class="mood-hint" id="noteWSaved"></div>
            </div>
            <div class="mood-hint" id="noteWHint"></div>
          </div>
        </div>

        <div class="mood-hint" style="margin-top:10px;opacity:.8;">
          Notes are stored per day and per person. Husband can only edit the Husband note; Wife can only edit the Wife note.
        </div>
      </div>
    </div>
  `;
  ...
  overlay.addEventListener("click", (e) => { if (e.target === overlay) hideNoteViewer(); });
}
...
function openNoteViewer(dayId, data = {}) {
  ensureNoteViewer();

  const overlay = document.getElementById("noteOverlay");
  document.getElementById("noteTitle").textContent = `Daily Note — ${dayId}`;

  const hText = document.getElementById("noteHText");
  const wText = document.getElementById("noteWText");
  const hSave = document.getElementById("noteHSave");
  const wSave = document.getElementById("noteWSave");
  const hSaved = document.getElementById("noteHSaved");
  const wSaved = document.getElementById("noteWSaved");
  const hHint = document.getElementById("noteHHint");
  const wHint = document.getElementById("noteWHint");

  // Pull from either cache keys or raw firestore keys
  const hVal = (data && (data.husbandNote ?? data.husband_note)) ? (data.husbandNote ?? data.husband_note) : "";
  const wVal = (data && (data.wifeNote ?? data.wife_note)) ? (data.wifeNote ?? data.wife_note) : "";

  hText.value = hVal || "";
  wText.value = wVal || "";

  const canEditH = (activeRole === "husband");
  const canEditW = (activeRole === "wife");

  hText.disabled = !canEditH;
  hSave.disabled = !canEditH;
  wText.disabled = !canEditW;
  wSave.disabled = !canEditW;

  hHint.textContent = canEditH ? "" : "Only Husband can edit this.";
  wHint.textContent = canEditW ? "" : "Only Wife can edit this.";

  if (hSaved) hSaved.textContent = "";
  if (wSaved) wSaved.textContent = "";

  hSave.onclick = async () => {
    try {
      hSave.disabled = true;
      if (hSaved) hSaved.textContent = "Saving...";
      await saveNoteForDay(dayId, "husband", hText.value);
      if (hSaved) hSaved.textContent = "Saved ✓";
      const cached = monthCache.get(dayId) || {};
      cached.husbandNote = hText.value;
      monthCache.set(dayId, cached);
      setTimeout(() => { if (hSaved) hSaved.textContent = ""; }, 1200);
    } catch (e) {
      console.error(e);
      if (hSaved) hSaved.textContent = "Could not save";
    } finally {
  ...
  wSave.onclick = async () => {
    try {
      wSave.disabled = true;
      if (wSaved) wSaved.textContent = "Saving...";
      await saveNoteForDay(dayId, "wife", wText.value);
      if (wSaved) wSaved.textContent = "Saved ✓";
      const cached = monthCache.get(dayId) || {};
      cached.wifeNote = wText.value;
      monthCache.set(dayId, cached);
      setTimeout(() => { if (wSaved) wSaved.textContent = ""; }, 1200);
    } catch (e) {
      console.error(e);
      if (wSaved) wSaved.textContent = "Could not save";
    } finally {
...
if (auth) {
  ...
  onAuthStateChanged(auth, (user) => {
    ...
    setStatus(true, "Online");
    attachMonthListener();
    // Streak UI has been removed from the app.
    // The old streak listener uses a Firestore query that can require a
    // composite index. When that index is missing, Firestore throws an
    // uncaught error during app startup, which can stop the rest of the UI
    // (including the Mood modal) from working. So we keep it disabled.
