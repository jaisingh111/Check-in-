...
async function saveMoodForDay(dayId, role, emoji){
  if (!db || !ROOM_ID) throw new Error("Not connected");
  const key = role === "husband" ? "husbandMood" : "wifeMood";
  const dayRef = doc(db, "rooms", ROOM_ID, "days", dayId);
  await setDoc(dayRef, { dayId, [key]: String(emoji || "") }, { merge: true });
}
// ---------------- Year in Review ----------------
...
function handleDayTap(dayId) {
  ...
  btnMood.onclick = () => {
    ...
    openViewer(__pendingDayId, data);
    setTimeout(() => {
      document.getElementById("dcMoods")?.scrollIntoView({ behavior: "smooth", block: "start" });
    }, 50);
  };
...
function openViewer(dayId, data) {
  ...
  body.innerHTML = `
    <div class="dc-viewer-stack">
      ...
      <div class="dc-moods" id="dcMoods">
        <div class="dc-moods-grid">
          <div class="dc-mood-card">
            <div class="dc-mood-title">${mineMoodLabel}</div>
            <div class="dc-mood-picker" id="dcMoodPicker"></div>
            <div class="dc-mood-selected"><span id="dcMoodSelected">—</span></div>
            <div class="dc-mood-actions">
              <button class="dc-btn small primary" id="dcMoodSave">Save mood</button>
              <button class="dc-btn small" id="dcMoodClear">Clear</button>
              <span id="dcMoodSaved" class="dc-saved"></span>
            </div>
          </div>
          <div class="dc-mood-card">
            <div class="dc-mood-title">${theirsMoodLabel}</div>
            <div id="dcMoodTheirs" class="dc-mood-readonly muted">No mood yet.</div>
          </div>
        </div>
        <div class="dc-mood-help muted">Mood is stored per day and per person. You can only edit your own mood.</div>
      </div>
      </div>
    </div>
  `;
...
// Mood section (shared view, per-person edit)
  const mineMoodKey = activeRole === "husband" ? "husbandMood" : "wifeMood";
  const theirsMoodKey = activeRole === "husband" ? "wifeMood" : "husbandMood";

  const pickerEl = viewer.querySelector("#dcMoodPicker");
  const selectedMoodEl = viewer.querySelector("#dcMoodSelected");
  const theirsMoodEl = viewer.querySelector("#dcMoodTheirs");
  const moodSaveBtn = viewer.querySelector("#dcMoodSave");
  const moodClearBtn = viewer.querySelector("#dcMoodClear");
  const moodSavedEl = viewer.querySelector("#dcMoodSaved");

  let selectedMood = (dayData && dayData[mineMoodKey]) ? String(dayData[mineMoodKey]) : "";
  const theirsMood = (dayData && dayData[theirsMoodKey]) ? String(dayData[theirsMoodKey]) : "";

  if (theirsMoodEl){
    theirsMoodEl.textContent = theirsMood ? theirsMood : "No mood yet.";
  }

  function renderMoodPicker(){
    if (!pickerEl) return;
    pickerEl.innerHTML = "";
    for (const em of MOOD_EMOJIS){
      const b = document.createElement("button");
      b.type = "button";
      b.className = "dc-emoji" + (selectedMood === em ? " selected" : "");
      b.textContent = em;
      b.onclick = () => {
        selectedMood = em;
        if (selectedMoodEl) selectedMoodEl.textContent = selectedMood;
        // Toggle selected styling
        for (const child of pickerEl.querySelectorAll(".dc-emoji")){
          child.classList.toggle("selected", child.textContent === selectedMood);
        }
      };
  ...
  if (moodSaveBtn){
    moodSaveBtn.onclick = async () => {
      try{
        ...
        if (moodSavedEl) moodSavedEl.textContent = "Saving...";
        await saveMoodForDay(dayId, activeRole, selectedMood);

        // Update in-memory cache so UI stays consistent without reload
        const cached = monthCache.get(dayId) || {};
        cached[mineMoodKey] = selectedMood;
        monthCache.set(dayId, cached);

        if (dayData) dayData[mineMoodKey] = selectedMood;

        if (moodSavedEl) moodSavedEl.textContent = "Saved 💾";
        setTimeout(() => { if (moodSavedEl) moodSavedEl.textContent = ""; }, 1200);
      }catch(e){
        console.error(e);
        if (moodSavedEl) moodSavedEl.textContent = "Could not save";
      }finally{
  ...
  if (moodClearBtn){
    moodClearBtn.onclick = async () => {
      selectedMood = "";
      renderMoodPicker();
      if (moodSaveBtn) moodSaveBtn.click();
    };
  }
