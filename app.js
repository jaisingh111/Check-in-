...
async function saveMoodForDay(dayId, roleKey, emoji, text){
  const payload = { updatedAt: serverTimestamp() };
  if (roleKey === "husband") {
    payload.husbandMoodEmoji = emoji;
    payload.husbandMoodText = text || "";
    payload.husbandMoodUpdatedAt = serverTimestamp();
  } else {
    payload.wifeMoodEmoji = emoji;
    payload.wifeMoodText = text || "";
    payload.wifeMoodUpdatedAt = serverTimestamp();
  }
...
function handleDayTap(dayId) {
  ...
  btnMood.style.display = "inline-block";
  ...
  btnMood.onclick = () => {
    hideDayMenu();
    openViewer(__pendingDayId, data, { focusMood: true });
  };
...
function openViewer(dayId, data, opts = {}) {
  ensureDayMenu();
...
// Mood / sleep emoji
const moodH = data.husbandMoodEmoji || "—";
const moodW = data.wifeMoodEmoji || "—";
const moodHTxt = data.husbandMoodText || "";
const moodWTxt = data.wifeMoodText || "";
const emojiChoices = ["😴","😪","😊","😌","😵‍💫","🤒","😡","😍"];

function moodCard(roleLabel, roleKey, currentEmoji, currentText){
  const canEdit = (activeRole === roleKey);
  const picker = canEdit ? `
    <div class="dc-emoji-grid" data-role="${roleKey}">
      ${emojiChoices.map(e => `<button type="button" class="dc-emoji-btn" data-emoji="${e}">${e}</button>`).join("")}
    </div>
    <div class="dc-mood-inputs">
      <textarea class="dc-mood-text" data-role="${roleKey}" rows="2" placeholder="Optional note">${escapeHtml(currentText)}</textarea>
      <button type="button" class="dc-btn dc-save-mood" data-role="${roleKey}">Save mood</button>
    </div>
  ` : `
    <div class="dc-mood-readonly">Only ${roleLabel} can edit this.</div>
  `;
  return `
    <div class="dc-mood-card" data-role="${roleKey}">
      <div class="dc-mood-head">
        <div class="dc-mood-title">${roleLabel} mood</div>
        <div class="dc-mood-current"><span class="dc-mood-emoji">${currentEmoji}</span></div>
      </div>
      ${currentText ? `<div class="dc-mood-note">${escapeHtml(currentText)}</div>` : ``}
      ${picker}
    </div>
  `;
}
...
parts.push(`
  ...
  <div class="dc-section-title">Sleep Emoji Mood</div>
  <div class="dc-mood-wrap">
    ${moodCard("Husband","husband",moodH,moodHTxt)}
    ${moodCard("Wife","wife",moodW,moodWTxt)}
  </div>
`);
...
body.innerHTML = parts.join("");
  ...
  // Wire mood UI
  const focusMood = !!opts.focusMood;
  body.querySelectorAll(".dc-mood-card").forEach(card => {
    const roleKey = card.getAttribute("data-role");
    const emojiEl = card.querySelector(".dc-mood-emoji");
    const grid = card.querySelector(".dc-emoji-grid");
    if (grid) {
      grid.addEventListener("click", (ev) => {
        ...
        grid.querySelectorAll(".dc-emoji-btn").forEach(b => b.classList.toggle("selected", b === btn));
      });
    }
    const saveBtn = card.querySelector(".dc-save-mood");
    if (saveBtn) {
      saveBtn.addEventListener("click", async () => {
        const emoji = (emojiEl.textContent || "—").trim();
        const txtEl = card.querySelector(".dc-mood-text");
        const text = txtEl ? txtEl.value : "";
        try{
          await saveMoodForDay(dayId, roleKey, emoji, text);
          setStatus(true, "Mood saved ✅");
        } catch(err){
          console.error(err);
          setStatus(false, "Mood save failed");
          alert("Couldn't save mood. Please try again.");
        }
      });
    }
  });
  ...
  if (focusMood) {
    const el = body.querySelector(".dc-section-title");
