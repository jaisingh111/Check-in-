...
function openViewer(dayId, data) {
  ...
  // Notes (each person edits their own note; both can read both)
  try {
    const roleKey = String(activeRole || "").toLowerCase();
    const isHusband = roleKey === "husband";
    const myField = isHusband ? "husbandNote" : "wifeNote";
    const partnerField = isHusband ? "wifeNote" : "husbandNote";
    const myLabel = isHusband ? "Husband" : "Wife";
    const partnerLabel = isHusband ? "Wife" : "Husband";

    const myNote = data?.[myField] || "";
    const partnerNote = data?.[partnerField] || "";

    const notesWrap = document.createElement("div");
    notesWrap.className = "dc-notes";

    notesWrap.innerHTML = `
      <div class="dc-note-col">
        <div class="dc-note-label">${myLabel} note</div>
        <textarea id="dcMyNote" class="dc-note-text" rows="3" placeholder="Write something for this day…">${escapeHtml(myNote)}</textarea>
        <button id="dcSaveNote" class="dc-note-btn">Save note</button>
        <div class="dc-note-hint">Only your note is editable on this device.</div>
      </div>

      <div class="dc-note-col">
        <div class="dc-note-label">${partnerLabel} note</div>
        <div class="dc-note-read">${escapeHtml(partnerNote || "—")}</div>
      </div>
    `;

    dcViewerBody.appendChild(notesWrap);

    const saveBtn = notesWrap.querySelector("#dcSaveNote");
    const ta = notesWrap.querySelector("#dcMyNote");
    saveBtn?.addEventListener("click", async () => {
      if (!db) return alert("Not connected yet. Please try again in a moment.");
      saveBtn.disabled = true;
      saveBtn.textContent = "Saving…";
      try {
        const text = (ta?.value || "").trim();
        await setDoc(doc(db, "rooms", ROOM_ID, "days", dayId), { [myField]: text }, { merge: true });
        // update local cache so reopening is instant
        if (monthCache?.has(dayId)) {
          monthCache.set(dayId, { ...(monthCache.get(dayId) || {}), [myField]: text });
        }
        saveBtn.textContent = "Saved ✓";
        setTimeout(() => (saveBtn.textContent = "Save note"), 900);
      } catch (e) {
        console.error(e);
        alert("Couldn't save note. Check connection/rules and try again.");
    ...
    console.error("Notes UI error", e);
  }
