...
function setSelectedDay(dayId){
  ...
  // QA is shared (not per-day); listener is attached once after Firebase init.
}
...
function disableQA(msg){
  ...
  // disable buttons
  document.querySelectorAll('.saveQA, .clearQA, .qaClearRole').forEach(b => b.disabled = true);
}
function enableQA(){
  [...hQ, ...wQ].forEach(t => { if(t){ t.disabled = false; } });
  document.querySelectorAll('.saveQA, .clearQA, .qaClearRole').forEach(b => b.disabled = false);
  if(qaHintEl) qaHintEl.textContent = 'Tap Save to sync answers to the other device. Use Clear when you want to reset.';
}

// Store QA in a single shared Firestore doc (not per-day)
function qaRef(){
  if(!db || !ROOM_ID) return null;
  return doc(db, `rooms/${ROOM_ID}/qa/current`);
}
function qaField(role, q){
  // role is 'h' or 'w' (from HTML), q is '1'..'4'
  return `${role}${q}`;
}
function qaTextarea(role, q){
  const id = `${role}_q${q}`; // ids are h_q1..h_q4, w_q1..w_q4
  return document.getElementById(id);
}

let unsubscribeQA = null;
function attachQAListener(){
  const ref = qaRef();
  if(!ref){
  ...
  unsubscribeQA = onSnapshot(ref, (snap) => {
    if(!snap.exists()) return;
    const data = snap.data() || {};
    // write values without disrupting typing
    for(const role of ['h','w']){
      for(let q=1; q<=4; q++){
        const el = qaTextarea(role, q);
        if(!el) continue;
        setIfNotFocused(el, data[qaField(role, String(q))] || '');
      }
    }
  });
}

async function saveOne(role, q){
  const ref = qaRef();
  if(!ref) return;
  const el = qaTextarea(role, q);
  const val = (el?.value || '').trim();
  const field = qaField(role, q);
  try{
    await setDoc(ref, { [field]: val, updatedAt: serverTimestamp() }, { merge: true });
  }catch(e){
    console.error(e);
    alert('Could not save. Check Firebase rules + setup.');
  }
}
async function clearOne(role, q){
  const el = qaTextarea(role, q);
  if(el) el.value = '';
  await saveOne(role, q);
}
...
function wireQA(){
  ...
  // Save / Clear per question
  document.querySelectorAll('.saveQA').forEach(btn => {
  ...
  document.querySelectorAll('.clearQA').forEach(btn => {
    btn.addEventListener('click', async () => {
      const role = btn.getAttribute('data-role');
      const q = btn.getAttribute('data-q');
      await clearOne(role, q);
      btn.blur();
    });
  });

  // Clear all for each role (existing bottom buttons)
  if(hClearBtn) hClearBtn.addEventListener('click', async () => clearRole('h'));
  if(wClearBtn) wClearBtn.addEventListener('click', async () => clearRole('w'));
}
...
// QA (Daily Questions)
wireQA();
// Listener attaches after auth becomes available (see onAuthStateChanged).
  attachMonthListener();
  wireQA();
