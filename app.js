...
function pickMemoryReelItems(year, docsById){
  // Pick up to one "both" day per month (first one found)
  const picks = [];
  for (let m=0;m<12;m++){
...
function renderMemoryReel(year, docsById){
  ...
  const picks = pickMemoryReelItems(year, docsById);
  if (!picks.length){
  ...
  for (const id of picks){
    ...
    const thumbUrl = data.husbandUrl || data.wifeUrl;

    item.innerHTML = `
      <img class="reelThumb" alt="Memory from ${id}" src="${thumbUrl}">
      <div class="reelMeta">
        <div><strong>${id}</strong></div>
        <div class="muted">Tap to view</div>
      </div>
    `;
    item.addEventListener("click", () => openViewer(id, data));
    reviewReel.appendChild(item);
  }
}
