...
function renderMemoryReel(year, docsById){
  ...
  const picks = pickRandomMemories(docsById, 12);
  if (!picks.length){
  ...
  for (const id of picks){
    ...
    item.innerHTML = `
      <div class="reelThumbs">
        <img class="reelThumb2" alt="Husband photo ${id}" src="${data.husbandUrl}">
        <img class="reelThumb2" alt="Wife photo ${id}" src="${data.wifeUrl}">
      </div>
      <div class="reelMeta">
        <div><strong>${id}</strong></div>
        <div class="muted">Random memory ✨</div>
      </div>
    `;
