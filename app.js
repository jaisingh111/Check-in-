...
function qaRef(){
  return doc(db, 'rooms', ROOM_ID, 'days', 'qa');
}
}
...
function applyRoleAnswers(role, answers){
  const list = role === 'husband' ? hQ : wQ;
  list.forEach(t => {
    if(!t) return;
    const key = t.dataset.q;
    if(key && answers && Object.prototype.hasOwnProperty.call(answers, key)){
      t.value = answers[key] || '';
    }
  });
}
...
async function saveQAAnswers(role){
  if(!db || !ROOM_ID) return;
  const ref = qaRef();
  const field = role === 'husband' ? 'husbandAnswers' : 'wifeAnswers';
  const payload = { [field]: collectRoleAnswers(role), updatedAt: Date.now() };
  try{
...
function scheduleQASave(role){
  clearTimeout(saveTimers[role]);
  saveTimers[role] = setTimeout(() => saveQAAnswers(role), 250);
}
function scheduleQASave(role){
  clearTimeout(saveTimers[role]);
...
function wireQA(){
  // Save buttons (one per question)
  document.querySelectorAll('.saveQA').forEach(btn => {
    btn.addEventListener('click', () => {
      const role = btn.dataset.role;
      // save entire role's answers
      saveQAAnswers(role);
    });
  });

  // Clear buttons (one per question)
  document.querySelectorAll('.clearQA').forEach(btn => {
    btn.addEventListener('click', () => {
      const role = btn.dataset.role;
      const q = btn.dataset.q;
      const ta = document.querySelector(`textarea[data-role="${role}"][data-q="${q}"]`);
      if(ta) ta.value = '';
      saveQAAnswers(role);
    });
  });

  // Optional "clear all" buttons at bottom of each role card (keep existing if present)
  if(hClearBtn) hClearBtn.addEventListener('click', () => { hQ.forEach(t => t && (t.value='')); saveQAAnswers('husband'); });
  if(wClearBtn) wClearBtn.addEventListener('click', () => { wQ.forEach(t => t && (t.value='')); saveQAAnswers('wife'); });
}
...
todayBtn.addEventListener("click", () => {
  ...
  wireQA();
  await loadQAAnswers();
  // live sync so both devices see updates quickly
  onSnapshot(qaRef(), (snap) => {
    const data = snap.exists() ? snap.data() : {};
    applyRoleAnswers('husband', data.husbandAnswers || {});
    applyRoleAnswers('wife', data.wifeAnswers || {});
  });
