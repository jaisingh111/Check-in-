...
function ensureMoodViewer() {
  ...
  // Use the SAME modal system as the photo viewer (dc-view + dc-view-card)
  const viewer = document.createElement("div");
  viewer.id = "moodOverlay";
  viewer.className = "dc-view"; // IMPORTANT: this works with .dc-view.show
  viewer.innerHTML = `
    <div class="dc-view-card" role="dialog" aria-modal="true">
      <div class="dc-view-top">
  ...
  viewer.addEventListener("click", (e) => {
    if (e.target === viewer) hideMoodViewer();
  });
}
...
function openMoodViewer(dayId, data = {}) {
  ...
  cards.forEach((card) => {
    ...
    // Only the matching role can edit/save their section
    ...
    card.saveBtn.onclick = async () => {
      try {
        ...
        // Keep local cache in sync for immediate re-open
        if (data) {
