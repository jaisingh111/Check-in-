...
(function(){
  ...
  // Clear button under each question (clears that box and saves)
  const clearBtns = Array.from(qaWrap.querySelectorAll('.clearQA'));
  clearBtns.forEach(btn=>{
    btn.addEventListener('click', async ()=>{
      const roleKey = btn.dataset.role; // 'h' or 'w'
      const qNum = btn.dataset.q;       // '1'..'4'
      const textarea = qaWrap.querySelector(`textarea[data-role="${roleKey}"][data-q="${qNum}"]`);
      if(textarea) textarea.value = '';
      const role = roleKey === 'h' ? 'husband' : 'wife';
      const hint = btn.parentElement?.querySelector('.saveHint');
      btn.disabled = true;
      ...
      try{
        ...
        console.error(e);
        if(hint) hint.textContent = 'Could not clear';
        setTimeout(()=>{ if(hint) hint.textContent = ''; }, 3500);
      }finally{
        btn.disabled = false;
      }
  ...
  // Clear all answers for a role (bottom Clear button in each column)
  async function clearRole(roleKey){
    const role = roleKey === 'h' ? 'husband' : 'wife';
    const boxes = Array.from(qaWrap.querySelectorAll(`textarea[data-role="${roleKey}"]`));
    boxes.forEach(t => t.value = '');
    await saveRoleAnswers(role);
  }

  const hClear = document.getElementById('h_clear');
  const wClear = document.getElementById('w_clear');

  if(hClear){
    hClear.addEventListener('click', async ()=>{
      hClear.disabled = true;
      try{
        await clearRole('h');
      }catch(e){ console.error(e); alert('Could not clear right now.'); }
      finally{ hClear.disabled = false; }
    });
  }
  if(wClear){
    wClear.addEventListener('click', async ()=>{
      wClear.disabled = true;
      try{
        await clearRole('w');
      }catch(e){ console.error(e); alert('Could not clear right now.'); }
      finally{ wClear.disabled = false; }
    });
  }
