...
// -------------------- Mood / Sleep Emoji (per-day, per-person) --------------------
function ensureMoodViewer() {
  let el = document.getElementById("dcMoodViewer");
  if (el) return el;

  el = document.createElement("div");
  el.id = "dcMoodViewer";
  el.className = "dc-view";
  el.innerHTML = `
    <div class="dc-view-overlay" data-close="1"></div>
    <div class="dc-view-card">
      <div class="dc-view-head">
        <div class="dc-view-title" id="dcMoodTitle">Sleep Emoji Mood</div>
        <button class="dc-btn dc-btn-ghost" type="button" id="dcMoodClose">Close</button>
      </div>
      <div class="dc-view-body" id="dcMoodBody"></div>
    </div>
  `;
  document.body.appendChild(el);

  const closeBtn = el.querySelector("#dcMoodClose");
  const overlay = el.querySelector(".dc-view-overlay");
  closeBtn.addEventListener("click", hideMoodViewer);
  overlay.addEventListener("click", hideMoodViewer);

  return el;
}

function showMoodViewer() {
  const el = ensureMoodViewer();
  el.classList.add("show");
  document.documentElement.classList.add("dc-lock");
}
function hideMoodViewer() {
  const el = document.getElementById("dcMoodViewer");
  if (el) el.classList.remove("show");
  document.documentElement.classList.remove("dc-lock");
}

async function openMoodViewer(dayId) {
  ensureMoodViewer();
  const body = document.getElementById("dcMoodBody");
  const title = document.getElementById("dcMoodTitle");
  if (title) title.textContent = `Sleep Emoji Mood — ${dayId}`;

  // Load current values
  const ref = dayRefFor(dayId);
  const snap = await getDoc(ref);
  const data = snap.exists() ? snap.data() : {};

  const moodFields = {
    husband: { emoji: "hMoodEmoji", text: "hMoodText", label: "Husband mood" },
    wife: { emoji: "wMoodEmoji", text: "wMoodText", label: "Wife mood" },
  };

  const emojiChoices = ["😴","😪","🥱","🙂","😐","😫","🛌","🌙","💤","🔥"];

  function section(role) {
    const mine = role === activeRole;
    const f = moodFields[role];
    const savedEmoji = data[f.emoji] || "—";
    const savedText = data[f.text] || "";

    const pickerBtns = emojiChoices.map(e => `
      <button type="button" class="dc-emoji-btn" data-emoji="${e}" ${mine ? "" : "disabled"}>${e}</button>
    `).join("");

    return `
      <div class="dc-mood-section">
        <div class="dc-mood-head">
          <div class="dc-mood-label">${f.label}</div>
          <div class="dc-mood-saved">
            <span class="dc-mood-emoji" id="dcMoodSavedEmoji_${role}">${savedEmoji}</span>
            <span class="dc-mood-text" id="dcMoodSavedText_${role}">${escapeHtml(savedText)}</span>
          </div>
        </div>

        <div class="dc-mood-edit ${mine ? "" : "dc-disabled"}">
          <div class="dc-emoji-row" id="dcMoodPicker_${role}">${pickerBtns}</div>
          <textarea class="dc-input dc-mood-note" id="dcMoodNote_${role}" rows="2" ${mine ? "" : "disabled"}
            placeholder="${mine ? "Write a short note (optional)..." : "Only " + (role === "husband" ? "Husband" : "Wife") + " can edit this."}">${mine ? (savedText || "") : ""}</textarea>
          <div class="dc-row">
            <button class="dc-btn" type="button" id="dcMoodSave_${role}" ${mine ? "" : "disabled"}>Save mood</button>
            <button class="dc-btn dc-btn-ghost" type="button" id="dcMoodClear_${role}" ${mine ? "" : "disabled"}>Clear</button>
          </div>
        </div>
      </div>
    `;
  ...
  body.innerHTML = `
    <div class="dc-mood-wrap">
      ${section("husband")}
      ${section("wife")}
    </div>
  `;

  // Wire up interactions for the active role only
  const mineRole = activeRole;
  if (mineRole && moodFields[mineRole]) {
    const f = moodFields[mineRole];
    let selectedEmoji = data[f.emoji] || "";

    const picker = document.getElementById(`dcMoodPicker_${mineRole}`);
    const noteEl = document.getElementById(`dcMoodNote_${mineRole}`);
    const saveBtn = document.getElementById(`dcMoodSave_${mineRole}`);
    const clearBtn = document.getElementById(`dcMoodClear_${mineRole}`);

    // preselect
    if (picker) {
      for (const b of picker.querySelectorAll(".dc-emoji-btn")) {
        if (b.dataset.emoji === selectedEmoji) b.classList.add("selected");
      }
      picker.addEventListener("click", (ev) => {
        const btn = ev.target.closest(".dc-emoji-btn");
        if (!btn) return;
        selectedEmoji = btn.dataset.emoji || "";
        for (const b of picker.querySelectorAll(".dc-emoji-btn")) b.classList.remove("selected");
        btn.classList.add("selected");
      });
    }

    async function persist(emoji, text) {
      const payload = {};
      payload[f.emoji] = emoji || "";
      payload[f.text] = (text || "").trim();
      payload.updatedAt = serverTimestamp();
      await setDoc(ref, payload, { merge: true });

      // Update UI (both sides display)
      const se = document.getElementById(`dcMoodSavedEmoji_${mineRole}`);
      const st = document.getElementById(`dcMoodSavedText_${mineRole}`);
      if (se) se.textContent = emoji || "—";
      if (st) st.textContent = (text || "").trim();
    }

    if (saveBtn) {
      saveBtn.addEventListener("click", async () => {
        await persist(selectedEmoji, noteEl ? noteEl.value : "");
        toast("Mood saved 💜");
      });
    }
    if (clearBtn) {
      clearBtn.addEventListener("click", async () => {
        selectedEmoji = "";
        if (picker) for (const b of picker.querySelectorAll(".dc-emoji-btn")) b.classList.remove("selected");
