...
// Optional UI (some builds remove the streak card entirely)
...
async function onDayCellClick(dayId) {
  // If a photo already exists for this day, show a small choice: view or take/replace.
  const role = selectedRole;
  const roleUrl = getRolePhotoUrl(dayId, role);
  const anyUrl = roleUrl || getPreferredPhotoUrl(dayId);

  if (!anyUrl) {
    return openCameraForDay(dayId);
  }
  ...
  openDayActionModal({
    ...
    role,
    viewUrl: anyUrl,
    hasRolePhoto: !!roleUrl,
  });
}

function getRolePhotoUrl(dayId, role) {
  const d = DAY_DOCS.get(dayId);
  if (!d) return null;
  return role === "husband" ? d.husbandUrl : d.wifeUrl;
}

function getPreferredPhotoUrl(dayId) {
  const d = DAY_DOCS.get(dayId);
  if (!d) return null;
  return d.husbandUrl || d.wifeUrl || null;
}

function ensureDayActionUI() {
  if (document.getElementById("dayActionOverlay")) return;

  const overlay = document.createElement("div");
  overlay.id = "dayActionOverlay";
  overlay.className = "dc-modal-overlay dc-hidden";
  overlay.innerHTML = `
    <div class="dc-modal-card">
      <div class="dc-modal-title">Day options</div>
      <div class="dc-modal-sub" id="dayActionSubtitle"></div>
      <div class="dc-modal-actions">
        <button class="btn" id="dayActionViewBtn" type="button">View photo</button>
        <button class="btn primary" id="dayActionTakeBtn" type="button">Take / replace</button>
        <button class="btn subtle" id="dayActionCancelBtn" type="button">Cancel</button>
      </div>
    </div>
  `;
  ...
  overlay.addEventListener("click", (e) => {
    if (e.target === overlay) closeDayActionModal();
  });

  const cancel = overlay.querySelector("#dayActionCancelBtn");
  cancel.addEventListener("click", closeDayActionModal);

  const viewOverlay = document.createElement("div");
  viewOverlay.id = "photoViewerOverlay";
  viewOverlay.className = "dc-modal-overlay dc-hidden";
  viewOverlay.innerHTML = `
    <div class="dc-modal-card dc-photo-viewer">
      <img id="photoViewerImg" alt="Photo" />
      <div class="dc-modal-actions">
        <button class="btn primary" id="photoViewerCloseBtn" type="button">Close</button>
      </div>
    </div>
  `;
  document.body.appendChild(viewOverlay);
  viewOverlay.addEventListener("click", (e) => {
    if (e.target === viewOverlay) closePhotoViewer();
  });
  viewOverlay.querySelector("#photoViewerCloseBtn").addEventListener("click", closePhotoViewer);
}

function openDayActionModal({ dayId, role, viewUrl, hasRolePhoto }) {
  const overlay = document.getElementById("dayActionOverlay");
  const subtitle = document.getElementById("dayActionSubtitle");
  const viewBtn = document.getElementById("dayActionViewBtn");
  const takeBtn = document.getElementById("dayActionTakeBtn");

  overlay.dataset.dayId = dayId;
  overlay.dataset.role = role;
  overlay.dataset.viewUrl = viewUrl;

  subtitle.textContent = hasRolePhoto
    ? "You already have a photo for this day."
    : "A photo exists for this day (maybe the other role).";

  viewBtn.onclick = () => openPhotoViewer(viewUrl);
  takeBtn.onclick = () => {
    closeDayActionModal();
